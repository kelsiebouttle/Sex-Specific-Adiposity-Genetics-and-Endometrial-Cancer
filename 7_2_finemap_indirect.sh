...
mkdir -p finemapping_results_indirect_v2
temp_ld_dir="temp_ld_matrices_indirect_v2/${region_id}_chr${chr}"
mkdir -p "${temp_ld_dir}"
...
/working/lab_tracyo/kelsieB/SuSiEx/bin/SuSiEx \
    --sst_file=Indirect_Effects_Clean.txt \
    --n_gwas=38430 \
    --ref_file="${temp_ld_dir}/chr${chr}_region_ref" \
    --ld_file="${temp_ld_dir}/chr${chr}_region_LD" \
    --out_dir=./finemapping_results_indirect_v2/ \
    --out_name=indirect_${region_id}_${lead_snp} \
    --chr=${chr} \
    --bp=${start},${end} \
